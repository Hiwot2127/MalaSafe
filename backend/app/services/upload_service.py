...
class UploadService:
    ...
    async def process_weekly_malaria_upload(
        ...
        Returns:
            ...
            Weekly uploads never dispatch the monthly close pipeline; the close
        ...
        # Parse CSV
        t0 = _now_ms()
        df, file_errors, row_errors = MalariaCSVParser.parse_weekly_data(file_content)

        if df is None or file_errors:
            stages.append(_stage("parse", "failed", t0, detail=file_errors[0].get("error") if file_errors else "empty"))
        ...
        for idx, row in df.iterrows():
            ...
            if not is_valid:
                validation_errors.append({
                    "row": row_num, "column": "district_code", "value": district_code, "error": error_msg
                })
                skipped_count += 1
                continue
        ...
        return success, message, len(df), created_count, skipped_count, validation_errors, str(file_id), None, None, stages
