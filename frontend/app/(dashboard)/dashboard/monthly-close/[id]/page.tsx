    ...
    return (
      <div className="mx-auto flex max-w-5xl flex-col gap-8 py-8">
        <Link 
          ...
          aria-label="Back to monthly close list"
        >
          <ArrowLeft className="size-4" aria-hidden /> back to monthly close
        </Link>
      ...
      <tbody>
        {rows.map((r) => (
          <tr key={r.id} className="border-b border-border/60 last:border-0">
            ...
            <td className="px-4 py-2 text-right font-mono text-sm tabular-nums">{r.actual_positive}</td>
            <td className="px-4 py-2 text-right font-mono text-sm tabular-nums">{r.predicted_positive ?? "-"}</td>
